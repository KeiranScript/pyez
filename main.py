from modules.deleter import delete_from_csv, delete_file, append_to_csv, CSV_FILE
from modules.login import login, read_config, save_config, CONFIG_DIR
from modules.requests import upload, shorten
from pathlib import Path
import typer
import csv

app = typer.Typer(no_args_is_help=True, help="E-Z.gg CLI", add_completion=False, add_help_option=False)

@app.command(name="print-config", help="Print the path to the configuration file.")
def print_config():
...
def edit_config():
    ...
    for key in config.keys():
        new_value = typer.prompt(f"Enter new value for {key} (leave blank to keep current value)", default=config[key])
        if new_value:
...
def uploader(
        ...
        domain: str = typer.Option("i.e-z.gg", "--domain", "-d", help="Override the domain of the uploaded file.")
):
    if auth() is None or read_config()["API_KEY"] == "":
        typer.echo("Please login first (pyez login)")
        return
    ...
    imageUrl, rawUrl, deletionUrl, status_code = upload(read_config()["API_KEY"], file_path)

    parsed_url = urlparse(url=imageUrl)
    if domain.startswith("https://"): domain = domain.split("//")[1]
...
def shortener(
    ...
    if auth() is None or read_config()["API_KEY"] == "":
        typer.echo("Please login first (pyez login)")
        if verbose: typer.echo(f"Api key: {read_config()['API_KEY']}")
        return
    ...
    shortenedUrl, deletionUrl, status_code = shorten(read_config()["API_KEY"], url)

    append_to_csv(deletionUrl, shortenedUrl, verbose)

    typer.echo(f"Shortened to {shortenedUrl}")
    typer.echo(f"Delete: {deletionUrl}")
    if verbose: typer.echo(f"Status code: {status_code}")


@app.command(name="delete", help="Delete a file using its deletion URL")
def delete(index: int = typer.Argument(None, help="The index of the file to delete.")):
